...
(function () {
    ...
    style.textContent = `
        :host {
            ...
            --suggestion-bg: rgba(255, 255, 255, 0.98);
        ...
        *, *::before, *::after {
        ...
        @media (prefers-color-scheme: dark) {
            :host {
                ...
                --suggestion-bg: rgba(35, 35, 35, 0.98);
            }
        }
        ...
        #sidebar {
            ...
            overflow-y: hidden; /* Hide sidebar overflow */
            z-index: 2147483646;
            visibility: hidden;
            opacity: 0;
            pointer-events: none;
    ...
    async function render() {
        ...
        sidebar.appendChild(scrollContent);
        ...
        addrInput.type = 'text';
        addrInput.placeholder = 'Search...'; // Simplified placeholder
        ...
        try {
            ...
            scrollContent.appendChild(tSec);
            ...
            scrollContent.appendChild(bSec);

            // Add Address Bar at the bottom (sticky)
    ...
    // Check and update visibility based on fullscreen state
    function updateFullscreenVisibility() {
        // 1. Check Fullscreen API (standard elements)
        if (document.fullscreenElement) {
            floatBall.classList.add('visible');
            return;
        }

        // 2. Check Browser Window state via Background Script
        chrome.runtime.sendMessage({ action: 'getWindowState' }, (res) => {
            if (chrome.runtime.lastError) {
                console.error('[ZenSidebar] Error getting window state:', chrome.runtime.lastError);
                return;
            }
            if (res && res.state === 'fullscreen') {
                floatBall.classList.add('visible');
            } else {
    ...
    // Listen for window resize (often triggered by fullscreen changes)
    window.addEventListener('resize', updateFullscreenVisibility);

    // Listen for Web Fullscreen API changes
